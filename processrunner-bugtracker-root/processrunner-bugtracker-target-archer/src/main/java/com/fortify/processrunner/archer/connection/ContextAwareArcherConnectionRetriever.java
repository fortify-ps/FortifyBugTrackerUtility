	...
	implements IContextAware, IContextPropertyProvider 
{
	...
	public void addContextProperties(Collection<ContextProperty> contextProperties, Context context) {
		...
		contextProperties.add(new ContextProperty(IContextArcherConnectionProperties.PRP_APPLICATION_NAME, "Archer application name", context, StringUtils.isNotBlank(getApplicationName())?getApplicationName():"Read from console", false));
		...
		contextProperties.add(new ContextProperty(IContextArcherConnectionProperties.PRP_USER_DOMAIN, "Archer user domain, use 'undefined' if not defined", context, StringUtils.isNotBlank(auth.getUserDomain())?auth.getUserDomain():"Read from console", false));
	...
	protected void updateConnectionProperties(Context context) {
		...
		if ( getApplicationName()==null ) {
			setApplicationName(System.console().readLine("Archer Application Name: "));
		}
